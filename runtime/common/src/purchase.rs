...
//! Module to process purchase of TETs.
...
pub trait Config: fabric_system::Config {
	...
	/// The amount of purchased locked TETs that we will unlock for basic actions on the chain.
	type UnlockedProportion: Get<Permill>;
	/// The maximum amount of locked TETs that we will unlock.
...
/// All information about an account regarding the purchase of TETs.
...
pub struct AccountStatus<Balance> {
	...
	/// The amount of free TETs they have purchased.
	free_balance: Balance,
	/// The amount of locked TETs they have purchased.
...
decl_storage! {
	trait Store for Module<T: Config> as Purchase {
		// A map of all participants in the TET purchase process.
		Accounts: map hasher(blake2_128_concat) T::AccountId => AccountStatus<BalanceOf<T>>;
		// The account that will be used to payout participants of the TET purchase process.
		PaymentAccount: T::AccountId;
		// The statement purchasers will need to sign to participate.
		Statement: Vec<u8>;
		// The block where all locked tets will unlock.
...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		fn payout(origin, who: T::AccountId) {
			...
			Accounts::<T>::try_mutate(&who, |status: &mut AccountStatus<BalanceOf<T>>| -> DispatchResult {
				...
				if !status.locked_balance.is_zero() {
					...
					// We allow some configurable portion of the purchased locked TETs to be unlocked for basic usage.
		...
		/// Set the account that will be used to payout users in the TET purchase process.
		///
		...
		/// Set the statement that must be signed for a user to participate on the TET sale.
		///
		/// Origin must match the `ConfigurationOrigin`
		#[weight = T::DbWeight::get().writes(1)]
		...
		/// Set the block where locked TETs will become unlocked.
		///
		/// Origin must match the `ConfigurationOrigin`
		#[weight = T::DbWeight::get().writes(1)]
