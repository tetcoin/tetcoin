...
use tp_consensus::{Proposal, RecordProof};
use tp_inherents::InherentData;
use tp_runtime::traits::{DigestFor, HashFor};
...
/// Custom Proposer factory for Tetcoin
pub struct ProposerFactory<TxPool, Backend, Client> {
	inner: tc_basic_authorship::ProposerFactory<TxPool, Backend, Client>,
...
impl<TxPool, Backend, Client> ProposerFactory<TxPool, Backend, Client> {
	pub fn new(
		...
		ProposerFactory {
			inner: tc_basic_authorship::ProposerFactory::new(
				spawn_handle,
				client,
				transaction_pool,
				prometheus,
...
impl<TxPool, Backend, Client> tp_consensus::Environment<Block>
...
where
	...
	Client: 'static
		...
		+ Sync,
	Client::Api:
		BlockBuilderApi<Block> + ApiExt<Block, Error = tp_blockchain::Error>,
	Backend:
		'static + tc_client_api::Backend<Block, State = tp_api::StateBackendFor<Client, Block>>,
	// Rust bug: https://github.com/rust-lang/rust/issues/24159
	tp_api::StateBackendFor<Client, Block>: tp_api::StateBackend<HashFor<Block>> + Send,
{
...
pub struct Proposer<TxPool: TransactionPool<Block = Block>, Backend, Client> {
	inner: tc_basic_authorship::Proposer<Backend, Block, Client, TxPool>,
	overseer: OverseerHandler,
...
impl<TxPool, Backend, Client> tp_consensus::Proposer<Block> for Proposer<TxPool, Backend, Client>
where
	TxPool: 'static + TransactionPool<Block = Block>,
	Client: 'static
		+ BlockBuilderProvider<Backend, Block, Client>
	...
	tp_api::StateBackendFor<Client, Block>: tp_api::StateBackend<HashFor<Block>> + Send,
{
	type Transaction = tc_client_api::TransactionFor<Backend, Block>;
	type Proposal = Pin<Box<
		dyn Future<Output = Result<Proposal<Block, tp_api::TransactionFor<Client, Block>>, Error>> + Send,
	>>;
	type Error = Error;
	...
	fn propose(
		...
		async move {
			...
			inherent_data.put_data(
				tetcoin_primitives::v1::INCLUSION_INHERENT_IDENTIFIER,
...
// one of the wrapped errors (tp_inherents::Error) also
// don't impl std::error::Error, which breaks the thiserror derive.
#[derive(Debug)]
pub enum Error {
	Consensus(tp_consensus::Error),
	Blockchain(tp_blockchain::Error),
	Inherent(tp_inherents::Error),
	Timeout,
...
impl From<tp_consensus::Error> for Error {
	fn from(e: tp_consensus::Error) -> Error {
		Error::Consensus(e)
	}
}

impl From<tp_blockchain::Error> for Error {
	fn from(e: tp_blockchain::Error) -> Error {
		Error::Blockchain(e)
	}
}

impl From<tp_inherents::Error> for Error {
	fn from(e: tp_inherents::Error) -> Error {
		Error::Inherent(e)
	}
}
