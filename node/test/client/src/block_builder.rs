...
use tetcoin_test_runtime::{GetLastTimestamp, UncheckedExtrinsic};
...
pub trait InitTetcoinBlockBuilder {
	/// Init a Tetcoin specific block builder that works for the test runtime.
	///
	/// This will automatically create and push the inherents for you to make the block valid for the test runtime.
	fn init_tetcoin_block_builder(&self) -> tc_block_builder::BlockBuilder<Block, Client, FullBackend>;

	/// Init a Tetcoin specific block builder at a specific block that works for the test runtime.
	///
	/// Same as [`InitTetcoinBlockBuilder::init_tetcoin_block_builder`] besides that it takes a [`BlockId`] to say
	/// which should be the parent block of the block that is being build.
	fn init_tetcoin_block_builder_at(
		&self,
		at: &BlockId<Block>,
	) -> tc_block_builder::BlockBuilder<Block, Client, FullBackend>;
}

impl InitTetcoinBlockBuilder for Client {
	fn init_tetcoin_block_builder(
		&self,
	) -> BlockBuilder<Block, Client, FullBackend> {
		let chain_info = self.chain_info();
		self.init_tetcoin_block_builder_at(&BlockId::Hash(chain_info.best_hash))
	}
	...
	fn init_tetcoin_block_builder_at(
		...
		let mut inherent_data = tp_inherents::InherentData::new();
		...
		let minimum_period = BasicExternalities::new_empty()
			.execute_with(|| tetcoin_test_runtime::MinimumPeriod::get());
		...
		let provisioner_data = tetcoin_node_subsystem::messages::ProvisionerInherentData::default();
...
/// Tetcoin specific extensions for the [`BlockBuilder`].
pub trait BlockBuilderExt {
	/// Push a Tetcoin test runtime specific extrinsic to the block.
	///
	...
	fn push_tetcoin_extrinsic(&mut self, ext: UncheckedExtrinsic) -> Result<(), tp_blockchain::Error>;
}
